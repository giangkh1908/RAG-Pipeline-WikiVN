...
def rrf_fusion(
    ...
    Args:
        ...
        bm25_results: Results from BM25 search as
            (chunk_id, doc_id, bm25_score, full_text) tuples (sorted by score desc)
    ...
    # BM25 results now include (chunk_id, doc_id, score, full_text)
    for rank, (chunk_id, doc_id, _bm25_score, full_text) in enumerate(bm25_results):
        rrf_score = 1.0 / (k + rank + 1)
        rrf_scores[chunk_id] = rrf_scores.get(chunk_id, 0.0) + rrf_score
        # Add BM25-only results to chunk_map so they are not lost
